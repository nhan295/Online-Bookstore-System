    ...
    fetchBooks() {
      axios.get('http://localhost:3000/api/books')
        .then(response => {
          ...
          this.romantic = allBooks.filter(book => book => book.LS_ID === 1);
          this.adventure = allBooks.filter(book => book => book.LS_ID === 2);
          this.economic = allBooks.filter(book => book => book.LS_ID === 3);
        })
        .catch(error => {
